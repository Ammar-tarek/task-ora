...
class _CalendarScreenState extends State<CalendarScreen> {
  ...
  Future<void> _loadEvents() async {
    ...
    if (!mounted) return;
    setState(() {
      _events = data.map((e) => CalEvent(
        id:               e.id,
        title:            e.title,
        start:            e.start,
        end:              e.end,
        color:            e.clientId != null ? const Color(0xFFBA1A1A) : AppColors.primary,
        clientId:         e.clientId,
  ...
  Widget build(BuildContext context) {
    ...
    final canAddEvent = profile == null ||
  ...
  Widget build(BuildContext context) {
    return Column(children: [
      ...
      Container(
        ...
        child: Row(
          children: _dayLetters.map((l) => Expanded(
            child: Center(
          ...
          return IntrinsicHeight(
            child: Row(
              ...
              children: List.generate(7, (dow) {
                ...
                final cellDate = DateTime(focusedDate.year, focusedDate.month, dayNum);
                        ...
                        children: [
                          ...
                          Align(
                            ...
                            child: Container(
                              width: 26, height: 26,
                              decoration: BoxDecoration(
                                color: isToday ? AppColors.goldDark : Colors.transparent,
                          ...
                          ...dayEvents.take(2).map((e) => Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              border: Border(
                                  left: BorderSide(color: e.color, width: 2)),
                            ),
                            child: Text(
                              e.title,
...
class _AgendaForDay extends StatelessWidget {
  const _AgendaForDay({required this.day, required this.events, required this.onEventTap});
  final DateTime day;
...
class _WeekBodyState extends State<_WeekBody> {
  ...
  void initState() {
    ...
    final initialOffset = ((nowH - _startHour) * _rowH - 100).clamp(0.0, double.infinity);
                  ...
                  child: Column(children: [
                    Text(
                      ...
                      style: AppTextStyles.labelCaps.copyWith(
                        color: isToday ? AppColors.goldDark : AppColors.onSurfaceVariant,
                        fontSize: 10,
                      ),
                    ),
                    const SizedBox(height: 2),
                    ...
                    return Positioned(
                      ...
                      child: Padding(
                        ...
                        child: Text(
                          ...
                          style: AppTextStyles.dataSm.copyWith(fontSize: 10),
                  ...
                  child: Stack(
                    children: [
                      ...
                      ...dayEvents.where((e) => !e.isAllDay).map((e) {
                        final topFrac = (e.start.hour + e.start.minute / 60.0) - _startHour;
                        final durH = (e.end.difference(e.start).inMinutes / 60.0).clamp(0.5, 24.0);
...
class _DayBodyState extends State<_DayBody> {
  ...
  void initState() {
    super.initState();
    final nowH = DateTime.now().hour.clamp(_startHour, _endHour);
    final initialOffset = ((nowH - _startHour) * _rowH - 120).clamp(0.0, double.infinity);
    _scrollCtrl = ScrollController(initialScrollOffset: initialOffset);
  }
  ...
  Widget build(BuildContext context) {
    final hours = List.generate(_endHour - _startHour + 1, (i) => _startHour + i);
    final dayEvents = widget.eventsForDay(widget.day).where((e) => !e.isAllDay).toList();
    final allDayEvents = widget.eventsForDay(widget.day).where((e) => e.isAllDay).toList();
    final gridH = hours.length * _rowH;
          ...
          child: Row(children: [
            ...
            ...allDayEvents.map((e) => Expanded(
              child: Container(
                ...
                child: Text(e.title,
                  style: AppTextStyles.bodySm.copyWith(color: e.color, fontSize: 11),
                ...
                child: Stack(children: [
                  ...
                  ...dayEvents.map((e) {
                    ...
                    return Positioned(
                      ...
                      child: GestureDetector(
                        onTap: () => widget.onEventTap(e),
                        child: _TimeGridEvent(event: e, showAssignee: true),
                      ),
                    );
                  }),
                ]),
      ...
      child: Row(children: [
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(
              '${fmt(s)} – ${fmt(e)}',
              style: AppTextStyles.dataSm.copyWith(fontSize: 9, color: event.color.withValues(alpha: 0.8)),
            ),
          ]),
        ),
        if (showAssignee && event.assigneeInitials != null)
          Container(
            ...
            alignment: Alignment.center,
            child: Text(event.assigneeInitials!,
                style: AppTextStyles.labelMd.copyWith(fontSize: 8, color: event.color)),
          ),
      ]),
    );
  }
      ...
      child: Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Row(children: [
            ...
            Expanded(child: Text(event.title, style: AppTextStyles.headlineSm)),
            if (event.assigneeInitials != null)
              Container(
                width: 34, height: 34,
                decoration: BoxDecoration(
          ...
          if (event.roomName != null)
            _DetailRow(icon: Icons.meeting_room_outlined, text: 'Room: ${event.roomName}'),
          if (event.clientName != null)
            _DetailRow(icon: Icons.business_outlined, text: event.clientName!),
          ...
          if (event.assigneeInitials != null && event.attendeeNames.isEmpty)
            _DetailRow(icon: Icons.person_outline, text: 'Assigned to ${event.assigneeInitials}'),
          const SizedBox(height: 8),
          Row(children: [
            if (canEdit) ...[
              Expanded(
...
class _AddEventSheetState extends State<_AddEventSheet> {
  ...
  Future<void> _fetchRooms() async {
    try {
      ...
      final names = (data as List)
          ...
          .toList();
      if (mounted) setState(() { _rooms = names; _loadingRooms = false; });
    } catch (_) {
  ...
  Future<void> _save() async {
    ...
    final newEvent = CalEvent(
      ...
      color:            _selectedClient != null ? const Color(0xFFBA1A1A) : AppColors.primary,
      clientId:         _selectedClient?.id,
            ...
            Expanded(
              child: InkWell(
                ...
                child: InputDecorator(
                  ...
                  child: Text(_fmt(_startTime), style: AppTextStyles.bodyMd),
                ),
              ),
            ),
            const SizedBox(width: 12),
            Expanded(
              child: InkWell(
                ...
                child: InputDecorator(
                  decoration: InputDecoration(
                    labelText: 'End time',
                    prefixIcon: const Icon(Icons.access_time_filled, size: 18),
                    border: OutlineInputBorder(borderRadius: BorderRadius.circular(8)),
                    contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
                  ),
              ...
              children: _employees.map((e) {
                ...
                return FilterChip(
                  ...
                  onSelected: (v) => setState(() {
                    if (v) { _selectedEmployeeIds.add(e['id']!); }
                    else   { _selectedEmployeeIds.remove(e['id']!); }
                  }),
...
class _EditEventSheetState extends State<_EditEventSheet> {
  ...
  void initState() {
    ...
    _costCtrl    = TextEditingController(text: e.cost != null ? e.cost!.toStringAsFixed(2) : '');
    _startTime   = e.start;
    _endTime     = e.end;
  ...
  Future<void> _fetchClients() async {
    ...
    setState(() {
      ...
      if (widget.event.clientId != null) {
        try {
          _selectedClient = list.firstWhere((c) => c.id == widget.event.clientId);
        } catch (_) {
  ...
  Future<void> _save() async {
    ...
    final updated = CalEvent(
      ...
      cost:             cost,
      roomName:         _selectedRoom,
      attendeeNames:    selectedNames.isNotEmpty ? selectedNames : widget.event.attendeeNames,
      assigneeInitials: selectedNames.isNotEmpty
          ? selectedNames.first.substring(0, 1).toUpperCase()
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          if (_loadingClients)
            ...
            DropdownButtonFormField<ClientModel>(
              ...
              items: [
                const DropdownMenuItem(value: null, child: Text('— No client —')),
                ..._clients.map((c) => DropdownMenuItem(value: c, child: Text(c.companyName))),
              ],
              onChanged: (v) => setState(() => _selectedClient = v),
            ),
          const SizedBox(height: 12),
...
class _ManageRoomsSheetState extends State<_ManageRoomsSheet> {
  ...
  Future<void> _fetchRooms() async {
    ...
    try {
      ...
      if (mounted) setState(() { _loading = false; _error = 'Could not load rooms.'; });
    }
  }
  ...
  Future<void> _addRoom() async {
    ...
    try {
      await SupabaseService.adminClient
          .from('rooms')
          .insert({'name': name, 'capacity': capacity});
      _nameCtrl.clear();
      _capacityCtrl.clear();
      await _fetchRooms();
    } catch (_) {
      if (mounted) setState(() { _saving = false; _error = 'Failed to add room.'; });
      return;
    }
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        if (_loading)
          ...
          Padding(
            ...
            child: Text('No rooms yet. Add one above.',
