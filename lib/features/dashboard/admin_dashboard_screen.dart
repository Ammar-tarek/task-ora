...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
  ...
  Future<void> _load({bool animate = true}) async {
    ...
    if (profile == null) {
      if (mounted) setState(() { _error = 'No profile found'; _loading = false; });
      return;
    }
    ...
    } catch (_) { /* employees stays empty — shows "No data" */ }

    if (mounted) {
      setState(() {
        _stats       = stats ?? const DashboardStats(
          totalTasks: 0, doneTasks: 0, inProgressTasks: 0,
          totalEmployees: 0, presentToday: 0, totalRevenue: 0,
          ...
          if (_loading)
            ...
            SliverPadding(
              ...
              sliver: SliverList(delegate: SliverChildListDelegate([
                ...
                const SizedBox(height: 12),
                _ResponsiveQuickActions(screenWidth: screenW, actions: [
                  _QuickActionData('Kanban Board', Icons.view_kanban_outlined,
                    () => context.go('/tasks')),
                  _QuickActionData('Analytics', Icons.bar_chart,
                    () => context.push('/analytics')),
                  _QuickActionData('Attendance', Icons.how_to_reg_outlined,
                ...
                TSectionHeader(
                  title: 'Recent Tasks', action: 'See All',
                ...
                if (isAdmin) ...[
                  TSectionHeader(
                    title: 'Team Members', action: 'Manage',
...
List<_QuickActionData> _adminFinanceActions(BuildContext context) => [
  _QuickActionData('Finance', Icons.account_balance_wallet_outlined,
    () => context.go('/finance')),
  _QuickActionData('Expenses', Icons.receipt_long_outlined,
    () => context.push('/expenses')),
  _QuickActionData('Teams', Icons.group_outlined,
    () => context.push('/teams')),
];
...
class _ResponsiveQuickActions extends StatelessWidget {
  const _ResponsiveQuickActions({required this.screenWidth, required this.actions});
    ...
    if (screenWidth > 500) {
      return Wrap(
        ...
        children: actions.map((a) =>
          _QuickAction(label: a.label, icon: a.icon, onTap: a.onTap),
        ).toList(),
      );
    }
    return SingleChildScrollView(
      ...
      child: Row(
        children: actions.map((a) => Padding(
          ...
          child: _QuickAction(label: a.label, icon: a.icon, onTap: a.onTap),
        )).toList(),
      ),
    );
  }
          ...
          child: SizedBox(
            ...
            child: total > 0
                ? Row(children: [
                    ...
                    _barSegment(inProgress / total, AppColors.statusInProgress),
      ...
      child: Container(
        ...
        child: Row(children: [
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(task.title,
              style: AppTextStyles.labelMd, maxLines: 1,
              overflow: TextOverflow.ellipsis),
            const SizedBox(height: 4),
            Wrap(
              spacing: 6,
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(user.role, style: AppTextStyles.bodySm,
            maxLines: 1, overflow: TextOverflow.ellipsis),
        ])),
        TStatusChip(
