...
class _TaskDetailSheetState extends State<TaskDetailSheet> {
  ...
  static const _employeeAllowed = {
    'not_started', 'in_progress', 'employee_done', 'on_hold',
  };
  ...
  void initState() {
    ...
    _perms = TaskPermissions(profile ?? ProfileModel(
      id: '', role: 'employee', fullName: '',
      status: 'active', timezone: 'UTC',
      preferredLanguage: 'en', createdAt: '',
    ));
    _load();
  }
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (taskData == null) { _error = 'Task not found'; return; }
  ...
  Future<void> _saveFullEdit() async {
    ...
    try {
      ...
      await TaskRepository.updateTaskAssignees(
        widget.taskId, _selectedAssigneeIds, assignedBy: currentUid,
      );
  ...
  Future<void> _sendComment() async {
    ...
    try {
      ...
      final ok  = await TaskRepository.addComment(
        widget.taskId, uid, text, isInternal: _commentIsInternal,
      );
      if (ok) {
        _commentCtrl.clear();
  ...
  Widget _buildFullEditBody() {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Wrap(spacing: 8, children: [
        _PriorityChip('low', 'Low', AppColors.statusLow, _priority,
            (v) => setState(() => _priority = v)),
        _PriorityChip('medium', 'Medium', AppColors.statusMedium, _priority,
            (v) => setState(() => _priority = v)),
        _PriorityChip('high', 'High', AppColors.statusHigh, _priority,
            (v) => setState(() => _priority = v)),
        _PriorityChip('critical', 'Critical', AppColors.error, _priority,
            (v) => setState(() => _priority = v)),
      ]),
      const SizedBox(height: 18),
      ...
      Wrap(
        ...
        children: _statusOptions.map((opt) => _StatusChip(
          opt.label, opt.displayLabel, _status,
      ...
      const SizedBox(height: 8),
      if (_clients.isEmpty)
        Text('No clients — create one in the Clients section.',
        ...
        Wrap(
          ...
          children: _selectedAssigneeIds.map((uid) {
            final emp = _allEmployees.firstWhere(
              (e) => e.id == uid,
              orElse: () => ProfileModel(
                id: uid, role: 'employee', fullName: 'Unknown',
                status: 'active', timezone: 'UTC',
                preferredLanguage: 'en', createdAt: '',
              ),
            );
  ...
  Widget _buildEmployeeBody() {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (_task!.assignees.isNotEmpty) ...[
        ...
        Wrap(
          spacing: 8, runSpacing: 8,
          children: _task!.assignees.map((a) => Row(
            mainAxisSize: MainAxisSize.min,
            children: [
      ...
      Wrap(
        ...
        children: _statusOptions
            .where((o) =>
                !o.isDefault || _employeeAllowed.contains(o.label))
            .map((opt) => _StatusChip(
                  opt.label, opt.displayLabel, _status,
                  (v) => setState(() => _status = v),
                ))
              ...
              IconButton(
                icon: Icon(
                  ...
                  color: _commentIsInternal
                      ? AppColors.primary : AppColors.onSurfaceVariant,
                  size: 18,
                ),
                tooltip: _commentIsInternal
                    ? 'Internal comment' : 'Visible to client',
                onPressed: () =>
                    setState(() => _commentIsInternal = !_commentIsInternal),
              ),
            _sendingComment
                ? const SizedBox(
                    width: 28, height: 28,
...
class _PriorityChip extends StatelessWidget {
  ...
  const _PriorityChip(
      this.value, this.label, this.color, this.selected, this.onSelect);
