...
class DashboardRepository {
  ...
  static Future<DashboardStats> fetchStats(ProfileModel profile) async {
    ...
    if (profile.isManager) {
      ...
      if (teamId != null) {
        tasksQuery = _safeQuery(client.from('tasks').select('id, status').or('team_id.eq.$teamId,handoff_to_team_id.eq.$teamId'));
      } else {
      ...
      final results = await Future.wait([
        tasksQuery,
        attendanceQuery,
        _safeQuery(admin.from('notifications').select('id').eq('recipient_id', profile.id).eq('is_read', false)),
      ]);
      ...
      final results = await Future.wait([
        ...
        _safeQuery(admin.from('profiles').select('id').neq('role', 'client').eq('status', 'active')),
        _safeQuery(admin.from('attendance').select('id').eq('attendance_date', today).eq('status', 'present')),
        _safeQuery(admin.from('crm_entries').select('paid_amount').eq('status', 'paid')),
        _safeQuery(admin.from('notifications').select('id').eq('recipient_id', profile.id).eq('is_read', false)),
      ]);
